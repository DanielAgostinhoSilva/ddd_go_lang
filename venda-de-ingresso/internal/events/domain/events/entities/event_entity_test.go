...
func (suite *EventSuitTest) SetupTest() {
	suite.props1 = &EventProps{
		Id:                "c381befe-e79e-498e-a237-fe5a8b0539c2",
		Name:              "Event Name A",
		...
		TotalSpots:        0,
		TotalSpotReserved: 0,
		PartnerId:         "1226f73f-ec21-4f23-a75b-9eb63caac5bb",
...
func (suite *EventSuitTest) Test_deve_inicializar_um_event() {
	...
	suite.Equal(suite.props1.PartnerId, event.GetPartnerId().Value().String())
...
func (suite *EventSuitTest) Test_deve_criar_um_evento_valido() {
	...
	suite.Equal(suite.createEventCommand.PartnerId, event.GetPartnerId().Value().String())
}
